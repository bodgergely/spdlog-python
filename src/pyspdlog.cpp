...
class syslog_sink : public Sink {
	public:
  syslog_sink(const std::string& ident = "", int syslog_option = 0, int syslog_facility = (1<<3), bool multithreaded = false ) {
    if(multithreaded){
...
public:
  SyslogLogger(const std::string &logger_name, const std::string &ident = "",
               bool multithreaded = false, int syslog_option = 0, int syslog_facilty = (1 << 3))
      : Logger(logger_name) {
    if (multithreaded) {
